...
def summarize_results(name, *results):
    ...
    if len(results) == 0:
        ...
        summary.failures = 0
        summary.code = 1
        warn(f"No result generated by test package {name}")
    else:
        nonemptyResults = list(filter(lambda r: r is not None, results))
        emptyResultCount = len(results) - len(nonemptyResults)
        if emptyResultCount > 0:
            warn(f"{emptyResultCount} test results are not available to "
                f"include in the summary of test package {name}; this may be "
                "due to a `run` function not returning the result of "
                "`run_modules` in the package's `__init__.py`.")

        summary.total = sum([result.total for result in nonemptyResults])
        summary.failures = sum([result.failures for result in nonemptyResults])
        if len(nonemptyResults) == 0:
            summary.code = 1  # Warning has been produced already
        else:
            summary.code = max([result.code for result in nonemptyResults])
    return summary
...
def run_suite():
    ...
    result = mainmod.run()
    if result is None:
        warn("no results returned by the main testsuite module")
        returnCode = 1
    else:
